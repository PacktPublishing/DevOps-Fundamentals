...
<tr class="[#if build.suspendedFromBuilding]Suspended[#elseif latestBuildSummary?has_content]${latestBuildSummary.buildState}[#else]NeverExecuted[/#if]">
    ...
    <td class="operations">
        ...
        [#if fn.hasPlanPermission("WRITE", build) ]
            <a id="editBuild:${build.key}" accesskey="E" href="${req.contextPath}/build/admin/edit/editBuildConfiguration.action?buildKey=${build.key}">[#if fn.isConfigurationReadOnly(build)][@ui.icon type="build-configure" textKey="chain.configuration.view"/][#else][@ui.icon type="build-configure" text="chain.configuration.edit"/][/#if]</a>
        [/#if]
        [#if user??]
