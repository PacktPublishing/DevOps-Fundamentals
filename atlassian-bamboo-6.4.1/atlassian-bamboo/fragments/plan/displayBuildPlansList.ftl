...
<!-- operations -->
    ...
    [#if fn.hasPlanPermission("WRITE", build) ]
        [#if fn.isConfigurationReadOnly(build)]
            <a id="${id}editBuild:${build.key}" accesskey="E" href="${fn.getPlanEditLink(build)}">[@ui.icon useIconFont=true type="edit" textKey="chain.configuration.view"/]</a>
        [#else]
            <a id="${id}editBuild:${build.key}" accesskey="E" href="${fn.getPlanEditLink(build)}">[@ui.icon useIconFont=true type="edit" textKey="chain.configuration.edit"/]</a>
        [/#if]
    [/#if]
