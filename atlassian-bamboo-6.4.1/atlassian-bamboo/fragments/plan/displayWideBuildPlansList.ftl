...
[#--used for jira bamboo plugin--]
[#macro showFullSummaryDetails summary]
[#-- @ftlvariable name="summary" type="com.atlassian.bamboo.resultsummary.ImmutableResultsSummary" --]
    <div class="details">
        <!-- Reason -->
...
[#macro planOperations build]
    ...
    [#if fn.hasPlanPermission('BUILD',build) ]
        [@ww.text var="stopAllTitle" name="build.stop.all" ][@ww.param]'${build.name}'[/@ww.param][/@ww.text]
        [@ww.text var="stopOneTitle" name="build.stop.name" ][@ww.param]'${build.name}'[/@ww.param][/@ww.text]
    ...
    [#if fn.hasPlanPermission("WRITE", build)]
        [#if fn.isConfigurationReadOnly(build)]
            <a id="editBuild:${build.key}" href="${fn.getPlanEditLink(build)}" >[@ui.icon useIconFont=true type="edit" textKey="chain.configuration.view"/]</a>
        [#else]
            <a id="editBuild:${build.key}" href="${fn.getPlanEditLink(build)}" >[@ui.icon useIconFont=true type="edit" textKey="chain.configuration.edit"/]</a>
        [/#if]
    [/#if]
...
[#-- @ftlvariable name="builds" type="java.util.Collection<com.atlassian.bamboo.plan.cache.ImmutableTopLevelPlan>" --]
    ...
    <tr>
        ...
        [#if hasBuildResults]
            ...
            [#if showBuildDetails]
                <td>
                    [#if (latestResultsSummary.successful || latestResultsSummary.failed) && latestResultsSummary.buildCompletedDate??]
