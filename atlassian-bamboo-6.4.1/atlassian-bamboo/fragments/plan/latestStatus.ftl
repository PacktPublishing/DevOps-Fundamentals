...
<div id="plan-status">
    [#if immutablePlan??]
        ...
        [#if numberOfRunningBuilds gte 1 ]
            [#if numberOfRunningBuilds == 1]
                [#list action.getCurrentlyBuildingPlans(statusSource.planKey) as currentlyRunning]
                    [@ww.url value='/browse/${currentlyRunning.buildIdentifier.buildResultKey}'  var="currentBuildUrl" /]
                ...
                [/#list]
            [#else]
                [@ww.url value='/browse/${statusSource.key}'  var="currentBuildUrl" /]
        ...
        [#elseif statusSource.latestResultsSummary??]
            ...
            [@ww.url var='latestBuildUrl' value='/browse/${latestBuildSummary.planResultKey}' /]
