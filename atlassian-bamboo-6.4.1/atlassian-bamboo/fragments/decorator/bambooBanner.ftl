<nav class="global" role="navigation">
    ...
    <div class="secondary">
        ...
        [#elseif ctx??]
            <li id="loginLink">
                <a id="login" href="${req.contextPath}/userlogin!doDefault.action?os_destination=${ctx.getCurrentUrl(req)?url}">[@ww.text name='bamboo.banner.login' /]</a>
            </li>
            [#if ctx.isEnableSignup() ]
                <li id="signupLink">
                    <a id="signup" href="${req.contextPath}/signupUser!doDefault.action">[@ww.text name='bamboo.banner.signup' /]</a>
                </li>
            [/#if]
        [/#if]
        </ul>
