define('page/deployment/deployment-project-versioning', [
    'jquery',
    'underscore',
    ...
    var deploymentProjectVersioning = (function() {
        var defaults = {
            ...
            options,
            $loading = $(widget.icons.icon({ type: 'loading aui-dialog-content-loading' })),
            showAddVariableDialog = function() {
                var $dialogContent = $('<div class="add-variable-dialog"/>').html($loading),
                    ...
                    success: function(variableNames) {
                        var variables = _.map(variableNames, function(variableName) {
                            return {
                                value: variableName,
                                label: variableName,
                                checked: _.contains(options.currentlySelectedVariables, variableName)
                            };
                        });
                        $(options.selectors.variableCheckboxList).empty().append(
                            options.variablesToIncrementItemTemplate({
                                variables: variables
                            })
                        );
                    },
    ...
    return deploymentProjectVersioning;
});
