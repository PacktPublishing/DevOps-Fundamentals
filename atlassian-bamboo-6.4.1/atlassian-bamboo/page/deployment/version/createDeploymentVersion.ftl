...
[@ww.form id='createDeploymentVersion' action='doCreateDeploymentVersion' namespace='/deploy' cancelUri='${returnUrl!"/deploy/viewDeploymentProjectEnvironments.action?id=${deploymentProject.id}"}']
    ...
    [#if action.hasPlanPermission('READ', linkedPlan)]
        ...
        ${soy.render("bamboo.deployments:view-deployment-version", "bamboo.page.deployment.version.showVersionNames", {
            ...
            "namingSchemeAppliesToBranches" : namingSchemeAppliesToBranches,
        ...
        [/@ww.param]
    [#else]
        [@ui.messageBox type='warning' titleKey='deployment.project.create.version.error.no.plan.permissions'/]
    [/#if]
[/@ww.form]
[#if action.hasPlanPermission('READ', linkedPlan)]
        ...
        require(['jquery', 'page/deployment/create-version-preview'], function($, createVersionPreview) {
            $(function() {
                createVersionPreview.init({
                    ...
                    namingSchemeAppliesToBranches: ${namingSchemeAppliesToBranches?c},
                    selectors: {
