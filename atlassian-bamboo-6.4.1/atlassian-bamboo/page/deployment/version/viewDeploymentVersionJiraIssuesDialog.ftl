...
[#else]
    ...
    <p class="view-more">[@ww.text name="deployment.execute.preview.version.issues.sublist"][@ww.param value=(pager.page.list.size())/][@ww.param]${pager.totalSize}[/@ww.param][/@ww.text]
        [#if comparingWithBuildResult]
            <a href="${req.contextPath}/deploy/viewCreateDeploymentVersionJiraIssues.action?planResultKey=${planResultKey}&deploymentProjectId=${deploymentProjectId}[#if compareWithVersionId > 0]&compareWithVersion=${compareWithVersion?html}&compareWithVersionId=${compareWithVersionId}[/#if]">
        [#else]
            <a href="${req.contextPath}/deploy/viewDeploymentVersionJiraIssues.action?versionId=${versionId}&compareWithVersion=${compareWithVersion?html}&compareWithVersionId=${compareWithVersionId}">
        [/#if]
