...
[#-- ============================================================================================= @ui.bambooSection --]
[#macro bambooSection step=0 title='' titleKey='' description='' descriptionKey='' dependsOn='' showOn=true tools='' cssClass='' headerWeight='h3' toolsContainer='div' sectionContainer='fieldset' id='' collapsible=false isCollapsed=true extraAttributes={}]
    [#if collapsible]
    ...
    <${sectionContainer}
        [#if id?has_content] id="${id?trim}"[/#if]
        [#if className?has_content] class="${className?trim}"[/#if]
        [#if style?has_content] style="${style?trim}"[/#if]
...
[#macro displayLink href title="" titleKey="" icon='' iconPath='' id='' cssClass='' data='' extraAttributes={}
    ...
    [#if inList]<li class="dropdown-item">[/#if][#t/]
        <a [#if !showText]title="${fn.resolveName(title, titleKey)}" [/#if]href="${href}" [#t/]
            ...
            [#if id?has_content]id="${id}" [/#if][#t/]
            [#if linkClass?trim?has_content || cssClass?trim?has_content]class="${linkClass}[#if cssClass?trim?has_content] ${cssClass}[/#if]" [/#if][#t/]
            ${fn.renderExtraAttributes(extraAttributes)}[#t/]
        >[#t/]
...
[#macro renderEnvironmentNameLink environment]
[#-- @ftlvariable name="environment" type="com.atlassian.bamboo.deployments.environments.DecoratedEnvironment" --]
    [#assign projectName=environment.getDeploymentProjectDisplayName() /]
    [#assign envName=environment.environmentDisplayName /]

    [#if environment.deploymentProject.operations.canView ]
        [@s.url action="viewDeploymentProjectEnvironments" namespace="/deploy" var="viewDeploymentProjectUrl"]
            [@s.param name="id" value=environment.deploymentProjectId /]
        [/@s.url]
        <a title="${environment.deploymentProject.name?html}" href="${viewDeploymentProjectUrl}">${projectName?html}</a>[#t]
    [#else]
        ${projectName?html}
    [/#if]
    ...
    [#if environment.operations.canView ]
        [@s.url action="viewEnvironment" namespace="/deploy" var="viewEnvironmentUrl"]
            [@s.param name="id" value=environment.id /]
        [/@s.url]

        <a title="${environment.name?html}" href="${viewEnvironmentUrl}">${envName?html}</a>[#t]
    [#else]
        ${envName?html}[#t]
    [/#if]
[/#macro]
...
[#macro displayAuthorAvatarForCommit commit avatarSize='24'][#rt]
    [#assign altText][#if commit.author?has_content]${fn.getAuthorFullName(commit.author)!}[/#if][#t][/#assign]
...
[#-- @ftlvariable name="author" type="com.atlassian.bamboo.author.AuthorContext" --]
...
[#macro displayAddJdkInline]
    [#if fn.hasGlobalAdminPermission()]
        [@ww.text var='addSharedJdkCapabilityTitle' name='builder.common.jdk.inline.heading' /]
        [@ww.url  var='addSharedJdkCapabilityUrl' value='/ajax/configureSharedJdkCapability.action' returnUrl=currentUrl /]
...
[#macro displayAddExecutableInline executableKey=""]
    [#if fn.hasGlobalAdminPermission()]
        [@ww.text var='addSharedBuilderCapabilityTitle' name='builders.form.inline.heading' /]
        [@ww.url  var='addSharedBuilderCapabilityUrl' value='/ajax/configureSharedBuilderCapability.action?builderKey=${executableKey}' returnUrl=currentUrl/]
...
[#--
    ...
    @optional format - 'short' or 'long' - non-relative dates can also use 'full' or 'datetime'
--]
...
[#-- ================================================================================================= @ui.asyncLink --]
[#--
    Creates a link that triggers an AJAX request when clicked.
...
--]
[#macro asyncLink id url label type='POST' returnUrl='' confirmationMessage='' cssClass='' icon='' tooltip='' disabled=false]
        <a id="${id}"
           class="asyncLink ${cssClass}
                 [#if disabled]
                     disabled
                 [/#if]
                 "
        ...
        >
            [#if icon?has_content]
                [@ui.icon type=icon useIconFont=true/]
            [/#if]
            ${label}
        </a>
    <script type="text/javascript">
            [#if !disabled]
            require(['widget/async-link'], function (AsyncLink) {
                new AsyncLink({
                    el: '#${id?js_string}',
                    url: '${url?js_string}',
                    type: '${type?js_string}',
                    confirmationMessage: '${confirmationMessage?js_string}',
                    returnUrl: '${returnUrl?js_string}'
                })
            });
            [/#if]
        AJS.$('#${id}').tooltip();
...
[#-- ============================================================================================ @ui.planBranchIcon --]
[#--
    Renders an icon to symbolize the given plan branch. It depends on the plan branching workflow which led to creation
    of the plan branch.
--]
[#macro planBranchIcon branch]
[#-- @ftlvariable name="branch" type="com.atlassian.bamboo.ww2.beans.DecoratedPlan" --]
    ${soy.render(
        "bamboo.web.resources.common:widget-plan-branch-selector",
        "bamboo.widget.plan.branch.selector.icon",
        { "workflow": branch.planBranchWorkflow.key }
    )}
[/#macro]

[#-- ================================================================================================ @ui.emptyState --]
[#--
    Renders empty state of a page, according to ADG standard.
--]
[#macro emptyState headerKey imageUrl descriptionKey='' description='']
    <div class="emptyState fullyCentered">
        <img class="emptyState__image" src="${imageUrl}" />
        <h1 class="emptyState__header">[@s.text name=headerKey /]</h1>
        [#if description?has_content]
            <p class="emptyState__description">${description}</p>
        [#else]
            <p class="emptyState__description">[@s.text name=descriptionKey /]</p>
        [/#if]
        <div class="emptyState__content">[#nested/]</div>
    </div>
[/#macro]
