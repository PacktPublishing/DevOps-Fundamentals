...
[@ww.url var='getBuildsUrl' namespace='/chain/admin/ajax' action='getChains' /]
[@ww.url var='getResultUrl' namespace='/chain/admin/ajax' action='getResult' /]
[@ww.url var='getChangesUrl' value='/rest/api/latest/result/' /]

[@ww.url var='unknownJiraType' value='/images/icons/jira_type_unknown.gif' /]
[@ww.text var='queueEmptyText' name='plan.liveactivity.noactivity' /]
            ...
            [#list chainExecution as execution]
                ...
                [#else]
                    ...
                    [#if chainExecution.startTime??]
                        ...
                        [/#assign]
                    [#else]
                        [@ww.text var="buildMessage" name="build.currentactivity.waiting"/]
                    [/#if]
                    [@chainListItem chainResultKey=execution.planResultKey cssClass=buildStatus triggerReason=execution.getTriggerReason().getNameForSentence()?html planKey=immutableChain.key buildMessage=buildMessage currentStage=currentStageText/]
                [/#if]
            [/#list]
    ...
    [#if stages?has_content]
        [@stageList id=id]
            [#list stages as stage]
                [@ww.url var="stageEditLink" value='/chain/admin/ajax/editStage.action' buildKey=key stageId=stage.id returnUrl=currentUrl /]
                [@ww.text var="menuTriggerText" name="menu.actions" /]
                [#local stageStatus]
                    [@ui.standardMenu triggerText=menuTriggerText id="actions-menu-${stage.id}" icon="configure" useIconFont=true subtle=true compact=true iconOnly=true]
                        ...
                        [#if fn.hasPlanPermission("ADMINISTRATION", chain)]
                            [@ww.url var='confirmDeleteStageLink' action='confirmDeleteStage' namespace='/ajax' buildKey=key stageId=stage.id returnUrl=currentUrl /]
                            <li>
                                [@ui.displayLink
                                    href=confirmDeleteStageLink
                                    titleKey="stage.delete"
                                    cssClass='stage-delete-link'
                                    extraAttributes={
                                        'data-title': 'Delete stage'
                                    }/]
                            </li>
                        [/#if]
                    [/@ui.standardMenu]
                ...
                [@stageListItem id=stage.id name=stage.name manual=stage.manual status=stageStatus description=stage.description cssClass="not-collapsible" finalStage=stage.final]
                    [@jobList]
                        [#list stage.jobs.toArray()?sort_by("name") as job]
                            [@ww.url var="editBuildConfigurationLink" action="editBuildConfiguration" namespace="/build/admin/edit" buildKey=job.key /]
                            [@ww.url var="editBuildRequirementLink" action="defaultBuildRequirement" namespace="/build/admin/edit" buildKey=job.key /]
                            [@jobListItem key=job.key name=job.buildName href=editBuildConfigurationLink editBuildRequirementLink=editBuildRequirementLink disabled=job.suspendedFromBuilding agentUnavailabilityHint=action.getAgentUnavailabilityHint(job)]
                                ...
                                [#if fn.hasPlanPermission("WRITE", chain) || fn.hasPlanPermission("ADMINISTRATION", chain)]
                                    <ul class="floating-toolbar">
                                        [#if fn.hasPlanPermission("WRITE", chain) ]
                                            <li>
                                                [#if job.suspendedFromBuilding]
                                                    [@ww.url var="resumeJobLink" namespace="/build/admin" action="resumeJob" buildKey=job.key returnUrl=currentUrl/]
                                                    [@ui.displayLink href=resumeJobLink titleKey="global.buttons.enable" mutative=true/]
                                                [#else]
                                                    [@ww.url var="suspendJobLink" namespace="/build/admin" action="suspendJob" buildKey=job.key returnUrl=currentUrl/]
                                                    [@ui.displayLink href=suspendJobLink titleKey="global.buttons.disable" mutative=true/]
                                                [/#if]
                                            </li>
                                        [/#if]
                                        [#if fn.hasPlanPermission("ADMINISTRATION", chain)]
                                            [@menu.displayPlanDeleteLink plan=job labelKey='global.buttons.delete' useIcon=false /]
                                        [/#if]
                                    </ul>
                                [/#if]
                            [/@jobListItem]
        ...
        [#else]
            ...
            <p>
                [@ww.url var="deployUrl"
                        action='newDeploymentProjectDetails'
                ...
                [#if fn.hasGlobalPermission("CREATE")]
                    [@cp.displayLinkButton cssClass="doNotDisableInPlanConfigReadOnlyMode" buttonId="deployLink" buttonLabel='deployment.project.button.create' buttonUrl=deployUrl/]
                [#else]
        ...
        <script type="text/javascript">
            ...
            var updateStage = function (data) {
                ...
                if (data.stage.finalStage) {
                    newTitleHTML = newTitleHTML + ' [@ui.lozenge text="final" /]';
                }
                        ...
                        [#if stage.manual]
                            [#if stage.status??]
                                [#if runnableStage && hasBuildPermission]
                                    [@ww.text var='customisedButtonLabel' name='chain.continue.parameterised.build']
                                        [@ww.param]${stage.name}[/@ww.param]
                                    [/@ww.text]
                                    [@ww.url value=stage.continueUrl var='continueStageUrl' returnUrl=currentUrl /]
                        ...
                        <ul>
                            [#list stage.children as job]
                                <li id="job-${job.key}" [#if job.description?has_content] title="${job.description}"[/#if] class="[#if (job.suspendedFromBuilding)!false]disabled [/#if][#if job.key == currentKey]active [/#if][#if job.status??]${job.status.displayClass}[/#if]" data-job-key="${job.key}">
                                    ...
                                    [/#if]
                                    [@ww.url value=navigationContext.getJobUrl(job) var='jobUrl'/]
...
[#macro branchNavigator navigationContext]
    ...
    [#assign createPlanBranchLink]
        [#assign parentObj = navObject.parent!navObject /]
        [#assign masterObj = parentObj.master!parentObj /]
        [#if fn.hasPlanPermission('ADMINISTRATION', masterObj)]
            [@s.url var="createBranchUrl" action="newPlanBranch" namespace="/chain/admin" planKey=navObject.key returnUrl=currentUrl /]
            <li class="create-plan-branch">
                [@ui.displayLink
                id="createBranchSidebar"
                titleKey="branch.create.new.title"
                href=createBranchUrl
                icon="add"
                useIconFont=true
                extraAttributes={"data-bamboo-analytics-event":"bamboo.create.plan.branch.sidebar.link"}
            /]
            </li>
        [/#if]
    [/#assign]
    ...
    [@ww.url value="/browse/{key}/editConfig" var='branchUrl'/]
    [#if !navObject.branches?has_content && !fn.hasPlanPermission('ADMINISTRATION', immutablePlan)]
           <p class="branches">[@ww.text name="chain.config.branches.nobranches"/]</p>
    [#else]
        [#if navObject.branches?has_content]
            [@branchList branches=navObject.branches urlToReplaceKey=branchUrl activeKey=currentKey uniqueIdPrefix="navBranch_" bottomElement=createPlanBranchLink/]
        [#else]
            <ul class="branches">
                ${createPlanBranchLink}
            </ul>
        [/#if]
    [/#if]
    <script type="text/javascript">
    ...
    [@dj.simpleDialogForm triggerSelector="#createBranchSidebar" width=710 height=500 headerKey="branch.create.new.title" submitCallback="BAMBOO.redirectToBranchRepositoryConfig" /]
[/#macro]

[#-- ============================================================================================ @chains.branchList --]

[#macro branchList branches urlToReplaceKey activeKey="" uniqueIdPrefix="" bottomElement=""]
    <ul class="branches">
        [#list branches?sort_by("displayName") as branch]
            ...
            <li[#if classString?has_content] class="${classString?trim}"[/#if]>
                [@ww.url value="/browse/${branch.key}/editConfig" var='branchUrl'/]
                [@ui.planBranchIcon branch=branch /]
...
[#macro statusRibbon navigationContext]
    ...
    [#local statuses = [{
        ...
        "specsSuccess": navObject.status.specsSuccess,
        "specsFailure": navObject.status.specsFailure,
...
[#macro stageListItem id name manual status description="" cssClass="" finalStage=false]
<li id="stage_${id}"[#if cssClass?has_content] class="${cssClass}"[/#if]>
    <div>
        <dl class="stageListGroup">
            <dt>${name}[#if description?has_content] <span>${description?html}</span>[/#if][#if finalStage][@ui.lozenge text="final" /][/#if][#if manual] [@ui.icon type='stage-manual' textKey="stage.manual.icon.description"/][/#if]</dt>
            <dd>${status}</dd>
        </dl>
        [#nested]
    </div>
...
[#macro createJobListItem chain stage]
    [#if fn.hasPlanPermission("ADMINISTRATION", chain)]
        ...
        <li class="create-job">
            ...
            [@ww.text var="contentAction" name="action.tip.job.add"]
...
[#macro noCreatePermissionsInfo]
<div class="no-create-chain-permissions-info">
...
[#function getPlanStatusHistoryDetails]
    [@ww.action name='planStatusHistory' namespace='/ajax' var='planStatusHistory' /]
