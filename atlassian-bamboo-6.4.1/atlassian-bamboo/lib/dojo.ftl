...
[#-- @ftlvariable name="url" type="String" description="Url of tooltip content, should be without contextname"--]
[#-- =================================================================================================== @dj.tooltip --]
...
[#-- ============================================================================================== @dj.tabContainer --]
[#macro tabContainer headings=[] headingKeys=[] selectedTab='' tabViewId='tabContainer' tabIds=[] analyticsEvent='']
...
<div id="${tabViewId}" class="aui-tabs horizontal-tabs">
    <ul class="tabs-menu">
        [#list resolvedHeadings as heading]
            [#assign href=tabIds[heading?index]!heading?replace(' ', '') /]
            <li class="menu-item [#if selected == heading] active-tab[/#if]">
                <a href="#${href}" data-heading="${heading}"
                [#if analyticsEvent?has_content]
                    [#assign analyticsEventPayload]
                        {[#t]
                            "name": "${analyticsEvent}",[#t]
                            "data": {"tabName": "${heading}"}[#t]
                        }[#t]
                    [/#assign]
                    data-bamboo-analytics-event='${analyticsEventPayload}'
                [/#if]
                ><strong>${heading}</strong></a>
            </li>
...
<script type="text/javascript">
    AJS.$(function ($) {
        ...
        $("#${tabViewId} > .tabs-menu > .menu-item > a").click(function() {
            AJS.Cookie.save("tabContainer.${tabViewId}.selectedTab", encodeURIComponent($(this).attr("data-heading")),365);
        });
            ...
            pageVisibilityManager.setUp(
                function (isInitialCall) {
                    ...
                    BAMBOO.panelTimeouts[panelId] = setTimeout(function () {
                        reloadPanel(panelId, '${url}', ${reloadEvery}, ${loadScripts?string}, null, ${callback?string});
                    }, isInitialCall ? ${reloadEvery} * 1000 : 0);
                },
                function () {
                    clearTimeout(BAMBOO.panelTimeouts[panelId]);
                }
