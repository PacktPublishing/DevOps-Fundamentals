...
[#-- @ftlvariable name="testSummary" type="com.atlassian.bamboo.resultsummary.tests.TestResultsSummary" --]
    [#if testSummary.newFailedTestCaseCount > 0]
        [@testsTable id="new-failed-tests" testsMap=testResults.newFailedTests showReason=false showJob=showJob showDuration=true showStackTrace=true
            isStackTraceExpanded=true caption=testsTableCaption('buildResult.tests.summary.newFailures.title', testSummary.newFailedTestCaseCount) /]
    [/#if]

    [#if testSummary.existingFailedTestCount > 0]
        [@testsTable id="existing-failed-tests" testsMap=testResults.existingFailedTests showReason=true showJob=showJob showDuration=true
            showFailingSince=true showStackTrace=true isStackTraceExpanded=false caption=testsTableCaption('buildResult.tests.summary.existingFailures.title', testSummary.existingFailedTestCount) /]
    [/#if]
                        ...
                        <tr id="testCase-${testResult.testCase.id}" class="test-case ${testResult.state.displayName?lower_case?html}[#if (testIndex) % 2 == 0] zebra[/#if] test-case-row-${testResult.testCase.id}[#if showStackTrace && hasStackTrace] [#if shouldExpandStackTrace]expanded[#else]collapsed[/#if][/#if]">
                            ...
                            <td class="quarantine-data">
                                [#if testResult.testCase.quarantineStatistics??]
                                    ...
                                    [#if quarantineUser??]
                                        [@ui.displayUserGravatar userName=quarantineUser.name size='18' class="avatar" alt="${quarantineUser.fullName!}"/]
                            ...
                            [#assign hasAdministrationPermission = fn.hasPlanPermission("ADMINISTRATION", immutablePlan) /]
                            [#assign hasQuarantinePermissions = fn.hasPlanQuarantinePermissions(immutablePlan)/]
                            [#if action.getUser()?? &&  (hasAdministrationPermission || hasQuarantinePermissions)]
                                <td class="actions">
                                    ...
                                    [@ui.standardMenu triggerText="" id="${id}-testResultActions-${testResult.testCase.id}" icon="configure" useIconFont=true subtle=true]
                                        ...
                                        [/#if]
                                        [#if showQuarantineActions && hasQuarantinePermissions && action.testQuarantineEnabled]
...
[#macro displayTestToJiraActions linkedJiraIssueKey testCaseId planResultKey returnUrl ]
    ...
    [@ww.url var="editCreateNewJiraIssueForTest" action="editCreateNewJiraIssueForTest" namespace="/ajax" testCaseId=testCaseId planResultKey=planResultKey returnUrl=returnUrl /]
    ...
    [#if linkedJiraIssueKey?has_content][#local buttonCssClass=buttonCssClass + " hidden"/][/#if]
    [@ww.url var="linkTestToJiraIssue" action="editLinkTestToJiraIssue" namespace="/ajax" testCaseId=testCaseId planKey=planResultKey.planKey returnUrl=returnUrl /]
...
[#macro displayTestInfo testSummary quarantineEnabled=true]
[#-- @ftlvariable name="quarantineEnabled" type="java.lang.Boolean" --]
[#-- @ftlvariable name="testSummary" type="com.atlassian.bamboo.resultsummary.tests.TestResultsSummary" --]
        ...
        [#if hasIgnoredTests]
            <li id="testsSummaryIgnored">
                ...
                [#if quarantineEnabled]
                    [@s.text name='buildResult.tests.summary.quarantined.or.ignored.summary']
                        [@s.param name="value" value=testSummary.ignoredTestCaseCount /]
                    [/@s.text]
                [#else]
                    [@s.text name='buildResult.tests.summary.ignored.summary']
                        [@s.param name="value" value=testSummary.ignoredTestCaseCount /]
                    [/@s.text]
                [/#if]
            </li>
