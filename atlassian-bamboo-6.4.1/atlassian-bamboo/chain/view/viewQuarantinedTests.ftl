            ...
            <table class="aui tests-table" id="quarantined-tests">
                <thead>
                    <tr>
                        ...
                        [#if featureManager.limitedTimeQuarantineEnabled]
                            <th>[@ww.text name="chain.quarantine.test.resumes" /]</th>
                        [/#if]
                        <th class="actions"></th>
                    </tr>
                </thead>
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <ol class="quarantine-test-history" style="margin-left: ${((10-testCaseHistory?size)* 12)}px;">
                                [#list testCaseHistory as pastResult]
                                    [@ww.url value="/browse/${pastResult.testClassResult.buildResultsSummary.planResultKey}/test/case/${pastResult.testCase.id}" var="testResultUrl"/]
                        ...
                        [#if featureManager.limitedTimeQuarantineEnabled]
                            <td>
                                [#if testCase.quarantineStatistics.quarantineExpiryDate??]
                                    [@ui.time datetime=testCase.quarantineStatistics.quarantineExpiryDate]${testCase.quarantineStatistics.quarantineExpiryDate?datetime?string}[/@ui.time]
                                [#else]
                                    -
                                [/#if]
                            </td>
                        [/#if]
                        <td class="actions">
                            [#if action.getUser()?? && fn.hasPlanQuarantinePermissions(immutablePlan)]
                                [@ui.displayLink
                                    titleKey="builder.common.tests.unquarantine"
                                    href="#"
                                    cssClass="aui-button unleash-test"
                                    extraAttributes={
                                        "data-plan-key": immutablePlan.key,
                                        "data-test-id": testCase.id
                                    }
                                    id="toggleTestIgnore-${testCase.id}"
                                    mutative=true
                                /]
                            [/#if]
                        </td>
