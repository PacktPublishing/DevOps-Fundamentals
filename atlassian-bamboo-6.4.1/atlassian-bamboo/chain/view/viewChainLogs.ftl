...
<html>
<head>
	[@ui.header pageKey='chainResult.logs.header' object='${immutablePlan.key} ${(resultsSummary.buildNumber)!}' title=true /]
...
[#if fn.isSpecsFailure(resultsSummary) || fn.isSpecsSuccess(resultsSummary)]
    <table class="aui chain-logs-table">
        <tbody>
            <tr class="expanded">
                <td>
                    [@s.text name='chainResult.specs.logs.header' /]
                </td>
                [#if action.specExecutionLogExists]
                    <td>
                        <a href="${req.contextPath}/download/${resultsSummary.planKey}/spec_logs/${resultsSummary.planResultKey}.log?disposition=attachment">[@ww.text name='chainResult.logs.download'/]</a> or
                        <a href="${req.contextPath}/download/${resultsSummary.planKey}/spec_logs/${resultsSummary.planResultKey}.log">[@ww.text name='chainResult.logs.view'/]</a>
                    </td>
                [#else]
            ...
            <tr class="log-trace">
                <td class="code" colspan="2">
                    <div>
                        <table id="specs-logs-${resultsSummary.planResultKey}">
                            <tbody>
                                [#list action.retrieveBambooSpecsExecutionLogs() as log]
    ...
    <div class="logs-table-container">
        <table id="job-logs" class="aui chain-logs-table">
            ...
            <th>[@ww.text name="chainResult.logs.table.job.header"/]</th>
            <th>[@ww.text name="chainResult.logs.table.logs.header"/]</th>
            </thead>
            <tbody>
