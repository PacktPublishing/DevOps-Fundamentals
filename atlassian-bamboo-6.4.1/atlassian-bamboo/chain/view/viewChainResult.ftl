...
[@ww.url var='manageElasticInstancesUrl' action='manageElasticInstances' namespace='/admin/elastic'/]
                    ...
                    [#if hasIgnoredTests][#compress ]
                        <li class="ignored">
                            <a href="[@ww.url value='/browse/${chainResult.planResultKey}/test/#ignored-tests' /]">
                                ${ignoredTestCaseCount} <span>[#if testQuarantineEnabled]
                                        [@ww.text name='chain.summary.test.quarantined.or.ignored' /]
                                    [#else]
                                        [@ww.text name='chain.summary.test.ignored' /]
                                    [/#if]
                                </span>
                            </a>
                        </li>
...
[#if chainResult.finished && chainResult.failed && testSummary.failedTestCaseCount == 0]
    ...
    [#if failedJobResults.size() == 1 ]
        ...
        [@ww.text var="errorSummaryHeader" name='buildResult.error.summary.chain.title' ]
...
[#if chainResult.inProgress]
    ...
    [@ps.showBuildLog id="buildResultSummaryLogs" /]
