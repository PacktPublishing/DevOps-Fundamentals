...
<div id="applicationError">
    <em>
       [#if errorDetails.buildSpecific]
           [#assign buildUrl = "/browse/" + errorDetails.buildKey /]
            ...
            [/#if]
            <a href="[@s.url value=buildUrl /]" rel="up">Build ${errorDetails.buildName?html} [#if errorDetails.buildNumber?? ] ${errorDetails.buildNumber}[/#if]</a>
    ...
    [@ui.renderWebPanels location='error.details' params={"error": errorDetails}/]
    ...
    [#if errorDetails.agentIds?has_content]
        ...
        [#list errorDetails.agentIdentifiers as agent]
            [#if agent.name?has_content]
                <a href="[@s.url action='viewAgent' namespace='/agent' agentId=agent.id /]" rel="up">${agent.name?html}</a>[#t]
            [#else]
                (${agent.id})
            [/#if]                
            [#if agent_has_next]
    ...
    <ul class="floating-toolbar">
        ...
        [#if fn.hasAdminPermission()]
            <li>
                <a href="[@s.url namespace='/' action='removeErrorFromLog' errorKey=errorDetails.buildKey error=errorDetails.errorNumber /]" class="remove mutative">
                    [@ui.icon type="delete" /]
