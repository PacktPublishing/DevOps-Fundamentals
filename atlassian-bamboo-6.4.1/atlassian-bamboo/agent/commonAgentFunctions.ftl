[#-- =================================================================================== @agt.executableEnvironments --]
[#--
    List of executable deployment environments
--]
[#macro executableEnvironments environmentList]
[#-- @ftlvariable name="environmentList" type="java.util.List<com.atlassian.bamboo.deployments.environments.DecoratedEnvironment>" --]
[#compress]
    [@ui.bambooPanel]
        [#if environmentList?has_content]
        <table class="aui">
            <caption>[@ww.text name='agent.environments.execute.description' /]</caption>
            <thead>
                <th>&nbsp;</th>
                <th>[@ww.text name='agent.environments.execute.header.environment' /]</th>
            </thead>
            <tbody>
                [#list environmentList as environment]
                <tr>
                    <td class="minColumn">${environment_index + 1}.</td>
                    <td>[@ui.renderEnvironmentNameLink environment=environment /]</td>
                </tr>
                [/#list]
            </tbody>
        </table>
        [#else]
        <p>[@ww.text name='agent.environments.execute.none' /]</p>
        [/#if]
    [/@ui.bambooPanel]
[/#compress]
[/#macro]
...
--]
[#macro executablePlans executableBuildables showLastBuild=false]
[#compress]
[#-- Executable Builds --]
    [@ui.bambooPanel]
        [#if executableBuildables?has_content]
        <table class="aui">
            <caption>[@s.text name='agent.builds.execute.description' /]</caption>
            <thead>
            <th>&nbsp;</th>
            <th>[@s.text name='plan.title' /]</th>
                [#if showLastBuild]
                <th>[@s.text name='agent.builds.execute.built.last.title' /]</th>
                [/#if]
            </thead>
            <tbody>
                    ...
                    [#if showLastBuild]
                        <td>
                            ...
                            [#if lastBuild?has_content]
                                [@s.text name='agent.builds.execute.built.last' ]
                                    [@s.param][@ui.icon type=lastBuild.buildState /][@ui.renderBuildResultSummary buildResultSummary=lastBuild /][/@s.param]
                                    [@s.param]${lastBuild.relativeBuildDate}[/@s.param]
                                [/@s.text]
                                ...
                                [#if lastBuild.failed]
                                    ...
                                    <span class="subGrey">
                                        [#if successBuild?has_content]
                                            [@s.text name='agent.builds.execute.built.last.success' ]
                                                [@s.param][@ui.renderBuildResultSummary buildResultSummary=successBuild /][/@s.param]
                                                [@s.param]${successBuild.relativeBuildDate}[/@s.param]
                                            [/@s.text]
                                        [#else]
                                            [@s.text name='agent.builds.execute.built.last.success.none' /]
                                        [/#if]
                                    </span>
                                [/#if]
                            [#else]
                                <span class="subGrey">
                                    [@s.text name='agent.builds.execute.built.last.none' /]
                                </span>
                            [/#if]
                        </td>
                    [/#if]
        ...
        [#else]
        <p>[@s.text name='agent.builds.execute.none' /]</p>
        [/#if]
    [/@ui.bambooPanel]
[/#compress]
[/#macro]
...
[#macro agentDetails headerKey, agentId, showOptions, refresh=false, showStatus='yes', descriptionKey='' ]
    [@ui.bambooPanel titleKey=headerKey ]
        ...
        [@ww.text name='agent.builds.successRatio.long' var="successRatePercentageTitle"]
    ...
    <div style="overflow: hidden;">
        ...
        <div>
            ...
            [/#if]
            [#if refresh]
                [@ww.url var='agentDetailsSnippetUrl' namespace='/ajax' action='viewAgentDetailsSnippet' escapeAmp=false
    ...
    [#-- buttons --]
        [#if showOptions?? && fn.hasAdminPermission()]
            [@ui.displayButtonContainer]
                [#if showOptions == 'all' ]
                    [@ww.url action='viewAgent' namespace='/agent' agentId='${agentId}' var='viewAgentUrl' /]
                    [@ww.url action='editAgentDetails' namespace='/admin/agent' agentId='${agentId}' returnUrl='${currentUrl}' var='editAgentDetailsUrl' /]
                    ...
                    [#if agent.type.identifier != "elastic"]
                        [@ww.url var="agentAssignmentUrl" action='configureAgentAssignments' namespace='/admin/agent' agentId='${agentId}' returnUrl='${currentUrl}' /]
                    ...
                    [#if buildAgent.active]
                        [#if buildAgent.enabled]
                            [@ww.url action='disableAgent' namespace='/admin/agent' agentId='${buildAgent.id}' returnUrl='${currentUrl}' var='disableAgentUrl' /]
                        ...
                        [#else]
                            [@ww.url action='enableAgent' namespace='/admin/agent' agentId='${buildAgent.id}' returnUrl='${currentUrl}' var='enableAgentUrl'/]
                    ...
                    [#if buildAgent.active && !buildAgent.requestedToBeStopped]
                        [@ww.url action='stopAgentNicely' namespace='/admin/agent' agentId='${buildAgent.id}' returnUrl='${currentUrl}' var='stopAgentNicelyUrl' /]
                    ...
                    [#switch agent.type.identifier]
                        [#case "local"]
                            [#if buildAgent.active && buildAgent.agentStatus.allowDelete]
                                [@ww.url action='stopAgent' namespace='/admin/agent' agentId='${agentId}' var='stopAgentUrl' /]
                            ...
                            [#elseif !buildAgent.active]
                                [@ww.url action='startAgent' namespace='/admin/agent' agentId='${agentId}' var='startAgentUrl' /]
                        ...
                        [#case "remote"]
                            [#if buildAgent.agentStatus.allowDelete]
                                [@ww.url action='removeAgent' namespace='/admin/agent' agentId='${buildAgent.id}' var='removeAgentUrl' /]
                                [@ww.text name='agent.delete.description' var='removeAgentTitleText'][@ww.param]${buildAgent.id}[/@ww.param][/@ww.text]
                ...
                [#elseif showOptions == 'short' ]
                    [@ww.url action='viewAgent' namespace='/admin/agent' agentId='${agentId}' var='viewAgentUrl' /]
...
[#macro header]
    ...
    [@ui.header page=heading description=agent.description!?html /]
[/#macro]
        ...
        <table id="capabilities-${group.typeKey}" class="capabilities aui">
            ...
            <tr>
                <th class="labelPrefixCell">
                    ...
                    [@ww.text name='agent.capability.type.${group.typeKey}.key.description' var='keyDescription' /]
                ...
                <th class="valueCell">
                    ...
                    [@ww.text name='agent.capability.type.${group.typeKey}.value.description' var='valueDescription' /]
...
[#macro executablesPaneContent action selector id isImageConfiguration = false]
<div id="loading-spinner" >[@ui.icon type="loading" /]</div>
[#local url]
    [#if isImageConfiguration]
        [#t][@ww.url action=action namespace="/agent/admin/ajax" configurationId=id /][#t]
    [#else]
        [#t][@ww.url action=action namespace="/agent/admin/ajax" agentId=id /][#t]
    [/#if]
[/#local]
<script type="text/javascript">
    require(['feature/agent-executable-pane'], function(ExecutablePane) {
        new ExecutablePane({
            url: "${url?js_string}",
            el: "${selector?js_string}"
        });
    });
</script>
[/#macro]
