...
<a href="#" onclick="toggleDebug('${parameters.id!"debug"}');return false;">[Debug]</a>
<div style="display:none" id="${parameters.id!"debug"}">
...
<table border="0" cellpadding="2" cellspacing="0" width="400" bgcolor="#DDDDDD">
    ...
    <tr>
        ...
        <#list stackObject.value.keySet() as propertyName>
            ...
            <td bgcolor="<#if (index % 2) == 0>#BBBBBB<#else>#CCCCCC</#if>"><#if stackObject.value.get(propertyName)??>${stackObject.value.get(propertyName).toString()}<#else>null</#if></td>
    </tr>
...
<table border="0" cellpadding="2" cellspacing="0" width="400" bgcolor="#DDDDDD">
    ...
    <tr bgcolor="<#if (index % 2) == 0>#BBBBBB<#else>#CCCCCC</#if>">
        <td>${contextKey}</td><td><#if stack.context.get(contextKey)??>${stack.context.get(contextKey).toString()}<#else>null</#if></td>
    </tr>
        <#assign index= index + 1>
    </#list>
</table>
