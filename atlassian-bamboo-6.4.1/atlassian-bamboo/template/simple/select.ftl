...
[/#if]
 class="[#rt/]
    [#if parameters.submitOnChange!'false' == 'true']
      submitOnChange [#t/]
    [/#if]
...
[/#if]
[#if parameters.headerKey2?? && parameters.headerValue2??]
...
[#if parameters.list??]
    [@ww.iterator value="parameters.list"]
            [#if parameters.groupBy?? || parameters.groupLabel??]
                [#if parameters.groupBy??]
                    [#assign groupLabel = stack.findValue(parameters.groupBy)/]
                [#elseif parameters.groupLabel??]
                    [#assign groupLabel = parameters.groupLabel/]
                [/#if]
                [#if parameters.i18nPrefixForGroup?has_content]
                    [#assign groupLabel][@ww.text name='${parameters.i18nPrefixForGroup?html}.${groupLabel}' /][/#assign]
                [/#if]
                [#if !currentGroupLabel?has_content && groupLabel?has_content]
                    [#assign currentGroupLabel = groupLabel/]
                    <optgroup label="${currentGroupLabel?html}">
                [#elseif currentGroupLabel?has_content && groupLabel?? && currentGroupLabel != groupLabel]
                    </optgroup>
                    [#assign currentGroupLabel = groupLabel/]
     ...
     class="uiSwitch${stack.findString(parameters.uiSwitch)?html}"[#rt/]
            [/#if]
            [#if optionTitleValue?has_content]
     data-dynamic-select-description="${optionTitleValue?html}"[#rt/]
            [/#if]
            [#if parameters.dataAttributeName?has_content && optionDataAttributeValue?has_content]
     data-${parameters.dataAttributeName?html}="${optionDataAttributeValue?html}"[#rt/]
            [/#if]
     ...
     selected="selected"[#rt/]
        ...
        [#if parameters.i18nPrefixForValue?has_content]
            [@ww.text name='${parameters.i18nPrefixForValue?html}.${itemValue?html}' /][#t]
        [#else]
            ${itemValue?html}[#t/]
        [/#if]
        </option>[#lt/]
