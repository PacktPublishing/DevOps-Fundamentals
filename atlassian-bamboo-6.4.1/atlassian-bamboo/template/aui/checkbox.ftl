...
labels
--]
<div class="field-group[#if parameters.fieldClass?has_content] ${parameters.fieldClass}[/#if]">
    [#if parameters.fieldLabel?? || parameters.fieldLabelKey??]
        <label[#if (parameters.id)?has_content] for="${parameters.id?html}" id="fieldLabel_${parameters.id}"[/#if]>[#t/]
            [#if parameters.fieldLabelKey??]
                [@ww.text name=parameters.fieldLabelKey /][#t/]
            [#else]
                ${parameters.fieldLabel}[#t/]
            [/#if]
        </label>[#t/]
    [/#if]
