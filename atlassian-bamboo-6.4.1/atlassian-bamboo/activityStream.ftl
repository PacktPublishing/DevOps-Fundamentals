...
<div id="${feedId}" class="activityFeed">
    <div class="header">
        <a id="feedLink-${feedId}" href="${feedUrl!}" class="feedLink" title="${feediconTitle}">${feediconTitle}</a>
    </div>
    [#if feedError??]
        <div id="feedError-${feedId}" class="error">
            <div class="inner">${feedError!}</div>
        </div>
    [/#if]
    [#if feedWarning??]
        <div id="feedWarning-${feedId}" class="warning">
            <div class="inner">${feedWarning!}</div>
        </div>
    [/#if]
...
<script type="text/javascript">
    ...
    AJS.$(function () {
        ...
        activityFeed_${feedId} = new ActivityFeed("${feedId}", "${baseurl}", "${baseurl}/download/resources/${resourceKey}", "${feedUrl!}", "${maxResults!}");
