...
<p class="label-sort">
    [@ww.url var='orderByAlphaUrl' action='viewLabels' namespace='/build/label' orderByRank='false']
        [#if project??]
        ...
        [/#if]
    [/@ww.url]
    [@ww.url var='orderByRankUrl' action='viewLabels' namespace='/build/label' orderByRank='true']
        [#if project??]
            [@ww.param name='projectKey']${project.key}[/@ww.param]
        [/#if]
        [#if build??]
