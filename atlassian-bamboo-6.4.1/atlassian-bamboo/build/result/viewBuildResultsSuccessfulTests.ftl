...
<ul id="submenu">
    ...
    [#if buildResults?? && buildResults.successfulTestResults?has_content && (pager.page.list)?has_content]
        ...
        [#if testSummary.fixedTestCaseCount > 0]
            [@tests.testsTable id="fixed-tests" testsMap=buildResultsSummary.filteredTestResults.fixedTests showReason=false showJob=false
                showDuration=true showStackTrace=false isStackTraceExpanded=false caption=tests.testsTableCaption('buildResult.tests.summary.fixed.title', testSummary.fixedTestCaseCount) /]
        [/#if]
...
[#-- @ftlvariable name="testResult" type="com.atlassian.bamboo.results.tests.TestResults" --]
