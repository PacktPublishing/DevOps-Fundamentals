...
<body>
    ...
    [#if buildResultsSummary?? && buildResultsSummary.filteredTestResults??]
        ...
        [#if testSummary.newFailedTestCaseCount gt 0]
            [@tests.testsTable id="new-failed-tests" testsMap=testResults.newFailedTests showReason=false showJob=false
            showDuration=true showStackTrace=true isStackTraceExpanded=true caption=tests.testsTableCaption('buildResult.tests.summary.newFailures.title', testSummary.newFailedTestCaseCount) /]
        [/#if]
        [#if testSummary.existingFailedTestCount gt 0]
            [@tests.testsTable id="existing-failed-tests" testsMap=testResults.existingFailedTests showReason=true showJob=false
            showDuration=true showFailingSince=true showStackTrace=true isStackTraceExpanded=false caption=tests.testsTableCaption('buildResult.tests.summary.existingFailures.title', testSummary.existingFailedTestCount) /]
        [/#if]
    [/#if]
</body>
</html>
